...
class ContractorReportsView(APIView):
    ...
    def post(self, request):
        serializer = ReportsNameConstructorSerializer(data=request.data, context={'constructor':request.user, 'request':request, 'company':request.user.company})
...
class ContractorReportView(APIView):
    ...
    def put(self, request, pk):
        ...
        serializer = ReportsNameConstructorSerializer(instance=instance, data=request.data, context={'constructor':request.user, 'request': request}, partial=True)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
