...
class UserReportReceivedView(APIView):
    ...
    def get(self, request):
        instances = ReportsName.objects.filter(user=request.user.id, constructor_accepted=True).order_by('-id')
        # Pagination logic
...
class UserReportReturnView(APIView):
    ...
    def get(self, request):
        instances = ReportsName.objects.filter(user=request.user.id, constructor_accepted=False).order_by('-id')
        # Pagination logic
        paginator = self.pagination_class()
        paginated_instances = paginator.paginate_queryset(instances, request)
        serializer = ReportsNamesSerializer(paginated_instances, many=True, context={'request':request})
...
class UserReportsView(APIView):
    ...
    def get(self, request):
        instances = ReportsName.objects.filter(user=request.user.id).order_by('-id')
        # Pagination logic
        paginator = self.pagination_class()
        paginated_instances = paginator.paginate_queryset(instances, request)
        serializer = ReportsNamesSerializer(paginated_instances, many=True, context={'request':request})
    ...
    def post(self, request):
        serializer = ReportsNameCreateSerializer(data=request.data, context={'user':request.user, 'request':request})
...
class UserReportView(APIView):
    ...
    def get(self, request, pk):
        instances = get_object_or_404(ReportsName, user=request.user.id, id=pk)
    ...
    def put(self, request, pk):
        instance = get_object_or_404(ReportsName, user=request.user.id, id=pk)
    ...
    def delete(self, request, pk):
        report_delete = get_object_or_404(ReportsName, user=request.user.id, id=pk)
